...
const NavItem = ({ label, hasIcon = false }) => (
  ...
  >
    {hasIcon && (
      <svg className="w-3.5 h-3.5 mr-0.5" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
  ...
  >
    <div className="px-5 py-4 flex flex-col divide-y divide-gray-100">
      {[
        ...
        <button
          key={label}
          onClick={() => navigate(MOBILE_ROUTES[label] ?? `/${label.toLowerCase().replace(/\s+/g, "-")}`)}
  ...
  return (
    <>
      <style>{`
        ...
        .get-started-nav:hover { background: #9631F5; }
        .contact-sales-nav {
          border: 1.5px solid #6E0ACE;
          color: #6E0ACE;
          transition: background 0.18s;
        }
        .contact-sales-nav:hover { background: #EFE0FE; }
      `}</style>

      <nav
        className={`fixed left-0 right-0 nav-root z-200 w-full bg-white h-18 ${
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-10 2xl:px-14">
          <div className="flex items-center justify-between h-[64px]">
            ...
            <div className="hidden lg:flex items-left gap-0 xl:gap-1">
              ...
              <button
                onClick={() => navigate(NAV_ROUTES["Integrations"])}
                className="flex items-center gap-[3px] text-[14px] font-thin text-[#323338] hover:text-[#6161ff] transition-colors duration-150 px-2 py-1 group whitespace-nowrap"
              >
                Integrations
            ...
            <div className="hidden lg:flex items-center gap-2 xl:gap-3">
              <button
                onClick={() => navigate(NAV_ROUTES["Pricing"])}
                className="text-[14px] font-thin text-[#323338] hover:text-[#6161ff] transition-colors px-2 py-1 whitespace-nowrap"
              >
                Pricing
              </button>

              <button
                onClick={() => navigate(NAV_ROUTES["Compare"])}
                className="text-[14px] font-thin text-[#323338] hover:text-[#6161ff] transition-colors px-2 py-1 whitespace-nowrap"
              >
                Compare
              </button>
              ...
              <button
                onClick={() => navigate(NAV_ROUTES["Contact Us"])}
                className="contact-sales-nav rounded-full px-[16px] py-[12px] text-[12px] font-thin whitespace-nowrap"
              >
                Contact Us
              </button>
              ...
              <button
                onClick={() => navigate("/book-a-demo")}
                className="get-started-nav rounded-full px-[16px] py-[12px] text-[12px] font-thin text-white whitespace-nowrap group"
              >
                Book a Demo{" "}
